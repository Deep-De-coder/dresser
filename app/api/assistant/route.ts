  ...
  try {
    ...
    if (!Array.isArray(items) || items.length > 200) {
      return NextResponse.json(
        ...
        if (!provider) {
          console.log("No provider available, using fallback");
          // Use intelligent fallback when no provider is available
          ...
          } else if (queryLower.includes('business') || queryLower.includes('meeting') || queryLower.includes('work') || queryLower.includes('office') || queryLower.includes('interview')) {
    ...
    try {
      ...
      return NextResponse.json(responseWithMeta);
          ...
          if (providerError.code === "OLLAMA_ERROR" || providerError.code === "CONNECTION_FAILED" || providerError.code === "TIMEOUT" || providerError.message?.includes("fetch failed")) {
         const queryLower = query.toLowerCase();
